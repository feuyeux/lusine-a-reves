...
class TTSProfile:
    ...
    def validate(self) -> None:
        ...
        if self.values.get("provider") == "cosyvoice2":
            ...
            if not 80 <= int(self.values.get("maxCharsPerChunk", 0)) <= 500:
                raise ValueError("CosyVoice2 maxCharsPerChunk must be between 80 and 500")
            if not 0 <= int(self.values.get("pauseMs", -1)) <= 1000:
                raise ValueError("CosyVoice2 pauseMs must be between 0 and 1000")
            if not -12.0 <= float(self.values.get("peakDbfs", 99)) <= -1.0:
                raise ValueError("CosyVoice2 peakDbfs must be between -12 and -1 dBFS")
