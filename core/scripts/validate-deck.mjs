...
for (const [index, slide] of (presentation.slides ?? []).entries()) {
  ...
  if (slide.audio) {
    try {
      const assetPath = getAudioAssetPath(slide.audio, args);
...
for (const [key, entry] of Object.entries(manifest)) {
  ...
  try {
    if (!fs.existsSync(getAudioAssetPath(key, args))) failures.push(`audio manifest points to missing asset: ${key}`);
  } catch {
