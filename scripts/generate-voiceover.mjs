...
const profilePath = resolveFromRoot(args.profile, "content/tts-profile.json");
...
if (provider === "cosyvoice2") {
  ...
  for (const slide of narratedSlides) {
    ...
    const output = getAudioAssetPath(audioPath);
...
for (const slide of narratedSlides) {
  ...
  const commandArgs = provider === "windows-sapi"
    ? [
      ...
      resolveFromRoot("scripts/synthesize-sapi.ps1"),
      "-Text", slide.narration,
      "-Output", temporary,
      "-Voice", voice,
      "-Rate", String(rate),
