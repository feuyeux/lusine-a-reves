...
const sample = JSON.parse(fs.readFileSync("content/presentation.json", "utf8"));
const sampleManifest = JSON.parse(fs.readFileSync("content/audio-manifest.json", "utf8"));
...
test("sample presentation satisfies the external deck contract", () => {
  const result = spawnSync(process.execPath, ["scripts/validate-deck.mjs"], { encoding: "utf8" });
  assert.equal(result.status, 0, result.stderr || result.stdout);
});

test("sample presentation has a manifest entry for every voiceover", () => {
  const manifestPath = "content/audio-manifest.json";
...
test("validation accepts a presentation supplied outside content/", () => {
